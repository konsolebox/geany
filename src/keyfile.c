...
static GPtrArray *keyfile_groups[SESSION + 1];
...
static void save_dialog_prefs(GKeyFile *config)
{
	...
	g_key_file_set_boolean(config, PACKAGE, "sidebar_folders_use_real_path", interface_prefs.sidebar_folders_use_real_path);
	...
	g_key_file_set_integer(config, PACKAGE, "smart_highlighting", editor_prefs.smart_highlighting);
	...
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_new_line", file_prefs.final_new_line_unless_empty);
...
static void load_dialog_prefs(GKeyFile *config)
{
	...
	interface_prefs.sidebar_folders_use_real_path = utils_get_setting_boolean(config, PACKAGE, "sidebar_folders_use_real_path", FALSE);
	...
	editor_prefs.long_line_enabled = utils_get_setting_boolean(config, PACKAGE, "long_line_enabled", editor_prefs.long_line_column > 0);
	editor_prefs.long_line_type = utils_get_setting_integer(config, PACKAGE, "long_line_type", GEANY_LONG_LINE_TYPE_LINE);
	if (editor_prefs.long_line_type == GEANY_LONG_LINE_TYPE_OLD) /* backward compatibility */
	{
		editor_prefs.long_line_type = GEANY_LONG_LINE_TYPE_DISABLED;
	...
	editor_prefs.smart_highlighting = utils_get_setting_integer(config, PACKAGE, "smart_highlighting", TRUE);
	...
	file_prefs.final_new_line_unless_empty = utils_get_setting_boolean(config, PACKAGE, "pref_editor_new_line", TRUE);
...
{
	GKeyFile *config = g_key_file_new();
	gchar *configfile = get_keyfile_for_payload(payload);
...
void configuration_open_files(GPtrArray *session_files)
{
	...
	while (file_prefs.tab_order_ltr ? ++i < (gint)session_files->len : --i >= 0);
